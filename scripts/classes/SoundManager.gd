...
func play_sound(stream: AudioStream, type: CHANNEL_TYPE, settings: StreamPlayerSettings = StreamPlayerSettings.new(), identifier: String = "") -> Node:
	match type:
		CHANNEL_TYPE.MONO:
...
func on_player_finished(player: Node, settings: StreamPlayerSettings):
	assert(player is AudioStreamPlayer or player is AudioStreamPlayer2D or player is AudioStreamPlayer3D)
	if settings.loop:
		player.play(settings.seek)
	else:
