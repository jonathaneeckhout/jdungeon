...
func launch_projectile(target_global_pos: Vector2, projectile_class: String, misc: Dictionary):	
	...
	if not projectile.is_misc_data_valid(misc):
		GodotLogger.error("The 'misc' data passed to this projectile does not match the expected keys.")
	...
	if G.is_server():
		...
		for entity: Node in watcher_component.watchers:
			...
			if target_id is int:
				sync_launch_to_client(target_node.peer_id, target_global_pos, projectile_class, misc)


func sync_launch_to_client(id: int, target_global_pos: Vector2, projectile_class: String, misc: Dictionary):
...
func launch_to_json(target_global_pos: Vector2, projectile_class: String, misc: Dictionary) -> Dictionary:
...
func _on_projectile_hit_object(object: Node2D, projectile: Projectile2D):
	...
	if projectile.collision_scene:
		...
		get_tree().create_timer(MAX_COLLISION_LIFESPAN).timeout.connect(coll_scene_instance.queue_free)
