...
func _effect_applied(_target: Node, _json_data: Dictionary):
	assert(stat_name_to_boost in StatsSynchronizerComponent.StatListPermanent + StatsSynchronizerComponent.StatListCounter)
	var targetStatusEffComp: StatusEffectComponent = G.world.get_entity_component_by_name(_target.get_name(), StatusEffectComponent.COMPONENT_NAME)
	var targetStatsComp: StatsSynchronizerComponent = G.world.get_entity_component_by_name(_target.get_name(), StatsSynchronizerComponent.COMPONENT_NAME)
	
	boost_ref = generate_boost(stat_name_to_boost, targetStatsComp.get(stat_name_to_boost), _json_data["stacks"])
...
func generate_boost(stat_to_boost: String, base_stat_value: int, stacks: int) -> Boost:
	...
	var boostValue: int = (base_stat_value + flatIncrease) + ((base_stat_value * modifierIncrease) - base_stat_value)
