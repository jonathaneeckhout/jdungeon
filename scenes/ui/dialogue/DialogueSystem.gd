...
func load_dialogue(dialogueRes: DialogueResource):
	...
	if Global.debug_mode:
		GodotLogger.info(
			"Loaded dialogue '{0}' with {1} snippets".format([ current_dialogue.dialogue_identifier, str(current_dialogue.dialogue_snippets.size()) ])
		)
...
func unload_dialogue():
	...
	if finishedDialogue:
		if Global.debug_mode:
			GodotLogger.info(
				"Dialogue '{0}' finished.".format([finishedIdentifier])
			)
...
func display_snipet(dialogueResource: DialogueResource, snippet: DialogueSnippetResource):
	...
	if Global.debug_mode:
		GodotLogger.info(
			"Displayed snippet with text '{0}'. Expected time for tween to finish at normal speed: {1}".format([ snippet.text, str(dialogueSize / letters_per_second) ])
		)
