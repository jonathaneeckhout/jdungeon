...
func get_speaker_portrait(id: int, portraitID: int) -> Texture:
	if abs(id) >= speaker_portraits.size():
		GodotLogger.warn(
			"Speaker ID {0} out of range. There's {1} speakers in this dialogue."
			).format([str(id), str(speaker_portraits.size())])
		return FAILSAFE_TEXTURE
		
	if abs(portraitID) >= speaker_portraits.size():
		GodotLogger.warn(
			"Portrait ID {0} out of range. There's {1} speakers in this dialogue."
	...
	if portrait is Texture:
		...
		GodotLogger.error(
			"Could not load the portrait {0} for speaker {1}"
			).format([str(portraitID), str(id)])
		return FAILSAFE_TEXTURE
...
func should_snippet_be_shown(snippet: DialogueSnippetResource, trueConditions: Dictionary) -> bool:
	...
	if not snippet.conditions_to_show.is_empty() and not trueConditions.has_all(snippet.conditions_to_show):
		return false
	
	#If ALL conditions_to_hide are true, do not show.
	if not snippet.conditions_to_hide.is_empty() and trueConditions.has_all(snippet.conditions_to_hide):
		return false
