...
func connect_cardinals(point_position) -> void:
	...
	for direction in directions:
		var cardinal_point :int = get_point(point_position + navigation_tilemap.local_to_map(direction))
...
func get_astar_path(start_position: Vector2, end_position: Vector2, max_distance := -1) -> Array:
	...
	var astar_path := _astar.get_point_path(get_point(map_start_pos), get_point(map_end_pos))
